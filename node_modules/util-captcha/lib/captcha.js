...
var colors = ['#D80D6A', '#07ACD8', '#D93822', '#00A18F'],
	imageTemplate = '../img/template.png',
	randxy = [{
		x: [1, 21, 41, 61],
		y: [30, 20, 30, 25]
	}, {
...
function create_pic(text, writeStream, fn) {
	var pic = gm(path.join(__dirname, imageTemplate))
		.font("Arial.ttf", 24);
 ...
 */
function create_pic_byType(text, writeStream, imageType, fn) {
	imageType = imageType || 'common';

	if (imageType === 'easy') {
		imageTemplate = '../img/template_easy.png';
	} else if (imageType === 'common') {
		imageTemplate = '../img/template.png';
	} else {
		imageTemplate = '../img/template_hard.png';
	}

	create_pic(text, writeStream, fn);
}
 ...
 * 创建图片验证码
 */
exports.create_pic_byType = create_pic_byType;

/**
 * 创建图片中间件，直接返回图片，验证码存在session中
 * @param  {Number} expires 过期时间(默认为30分钟)
 */
exports.mid_pic_session = function(expires, imageType) {
	return function(req, res) {
		var type = '_cc_pic_' + (req.query.type || 'default');
		create_pic_byType(null, res, imageType, function(code) {
			req.session[type] = {
				code: code,
 ...
 * @param  {Number} expires 过期时间(默认为30分钟)
 */
exports.mid_pic_redis = function(client, key, expires, imageType) {
	return function(req, res) {
